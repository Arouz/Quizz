...
/* else if(preg_match('/^\/qtest/', $Message)) {
    test($Id_du_chat, $Pseudo_du_joueur, $Id_du_joueur, $Nom_du_joueur);
} */
else {
...
function StartQuizz($Id_du_chat) {
    ...
    if ($qdtr){
        ...
        if ($TailleTableau <= $NombreQuestion) {
            ...
            $QDTR = 'UPDATE `QuizzParties` SET `QDT`=:QDT WHERE IDR=:IDR';
            ...
            // Selectionne la question avec l'id tirée au hasard
            $select_q = "SELECT * FROM `QuizzQuestions` WHERE ID=:ndq";
            $stmt = $db->prepare($select_q);
            $stmt->bindParam(':ndq', $NumeroQuestion);
            $stmt->execute();
            ...
            $emoteQ = "\u2753";
            sendMessage($Id_du_chat, json_decode('"'.$emoteQ.'"').$Q);

            if ($P != "")
                sendPhoto($Id_du_chat, $P);

            // Met à jour la table quizzencours avec la question sélectionnée
            $QECU = 'UPDATE `QuizzEnCours` SET `Question`=:Q, `Reponse`=:R1 WHERE IDR=:IDR';
            $qecur = $db->prepare($QECU);
            $qecur->bindParam(':IDR', $Id_du_chat);
            $qecur->bindParam(':Q', $Q);
            $qecur->bindParam(':R1', $R1);
            $qecur->execute();

        } else {
            sendMessage($Id_du_chat, "All the questions have fallen.");
        }

    } else {
        sendMessage($Id_du_chat, 'No game in this room have been found.'); 
    }
...
function Reponse($Id_du_chat, $Message, $Pseudo_du_joueur, $Id_du_joueur, $Nom_du_joueur) {
    ...
    if ($QuestionReponse) {
        ...
        // Si la réponse du joueur correspond à 70% de la bonne réponse alors le point lui est accordé
        for ($i=0; $i < $TailleTableauRep; $i++) { 
            ...
            if ($Mess == $Reps)
                $Bonnerep = true;   
            else if ($pourcent > 70 && count($Reps) > 6)
                $Bonnerep = true;
        }
        ...
        if ($Bonnerep == false) {
            ...
            if ($TTMRE > 0 || $TTMRV > 0){
                for ($l=0; $l < $TTBRM; $l++) { 
                    ...
                    //sendMessage($Id_du_chat, '$TTMRE > $TTMRV');
                    ...
                    for ($i=0; $i < $TTM; $i++) { 
                        for ($j=0; $j < $TTBR ; $j++) { 
                            ...
                            //sendMessage($Id_du_chat, $MRE.' '.$MBR);

                            if ($MR == $MBR) {

                                //sendMessage($Id_du_chat, 'Simili');
                                ...
                                if ($SARDJ > 0) {
                                    ...
                                    $key = array_search($MR, $Array_Reponse_du_joueur); // $key = 2;
                                    if ($key != false){
        ...
        if ($Bonnerep && $JNBT > 0){
            ...
            sendMessage($Id_du_chat, json_decode('"'.$EmoteGG.'"').' Good answer '.$NmduJoueur.' !');
...
function information($Id_du_chat){
    ...
    sendMessage($Id_du_chat, ' [Note] some questions can be difficult especially those where you have to enumerate several characters.');
}

function test($Id_du_chat, $Pseudo_du_joueur, $Id_du_joueur, $Nom_du_joueur) {
    require "../lib/connexionBase.php";
    $db = connexionBase();
